      ...
      <section className="py-20 md:py-28 px-4 md:px-6 bg-white">
        <div className="max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            <span className="text-amber-600 font-bold tracking-[0.2em] text-xs uppercase mb-4 block">Marketing & Media</span>
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tight uppercase">Campaign Banners</h2>
            <div className="h-1.5 w-20 bg-amber-400 rounded-full mx-auto mt-4"></div>
          </motion.div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { src: "/flex-1.jpg", title: "Campaign Landmark Banner 1", desc: "Bold India Group project marketing design." },
              { src: "/flex.jpg", title: "Campaign Landmark Banner 2", desc: "Physical board hoarding layout." },
              { src: "/j.jpg", title: "Brand Identity Banner", desc: "Landmark project highlight graphic." }
            ].map((banner, i) => (
              <motion.div 
                key={i} 
              ...
              >
                <div className="relative w-full h-[300px] md:h-[380px] rounded-[2rem] overflow-hidden border border-slate-100 bg-slate-950 mb-6 flex items-center justify-center shadow-lg">
                  <img 
                    src={banner.src} 
                    alt={banner.title} 
                ...
                <h3 className="text-xl font-bold text-slate-900 mb-2">{banner.title}</h3>
                <p className="text-slate-500 text-sm font-medium">{banner.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
